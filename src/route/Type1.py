...
class Type1():
    def __init__(self, graph_util, type1):
    ...
    def least_used_capacity_percentage(self, lambdak):
        ...
        for i, path in enumerate(paths):
            if self.graph.check_path_enough_capacity(path, Uk) == True:
                all_capacity = sum([self.graph.graph.capacity[(u, v)]
    ...
    def min_max_percentage(self, lambdak):
        ...
        for i, path in enumerate(paths):
            if self.graph.check_path_enough_capacity(path, Uk) == True:
                max_percentage = 0
                for u, v in zip(path, path[1:]):
                    if max_percentage < (Uk/self.graph.graph.capacity[(u, v)]):
                        max_percentage = (Uk/self.graph.graph.capacity[(u, v)])
